...
const quizInfo = {
  gsec: { title: "GIAC GSEC", desc: "セキュリティ基礎・認証・ネットワーク・暗号・防御運用" },
  gpen: { title: "GIAC GPEN", desc: "ペネトレーションテスト・情報収集・脆弱性検証・権限昇格・報告" },
  gcia: { title: "GIAC GCIA", desc: "侵入検知・パケット解析・IDS/IPS・ネットワーク監視" },
  gcih: { title: "GIAC GCIH", desc: "インシデント対応・ハンドリング・攻撃手法・封じ込め・復旧" }
};
...
if (type === "all") {
  document.title = "GIAC資格クイズ";
  pageTitle.textContent = "GIAC資格クイズ";
  pageDesc.textContent = "4カテゴリ・各150問から50問ランダムで出題";
} else {
  const info = quizInfo[type] || quizInfo.gsec;
